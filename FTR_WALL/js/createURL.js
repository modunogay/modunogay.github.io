	...
	$(function () {
		$.get("./version/now.txt", function (data) {
			$.getJSON(`./json/${data}.json`, function (list) {
				...
				str = str.join("_")
				var url = "http://" + location.host + "/FTR_WALL/index.html?" + str;
				$("#URL-Input").val(url);
			});
		})
	})
...
function setlocalstorage(Name, value, setName) {
	...
	if (setName == "FTR_WALL") {
	...
	}else if(setName == "FTR_WALL_TWITTER"){
		for (key in USER_JSON_FTR_WALL_TWITTER) {
			if (USER_JSON_FTR_WALL_TWITTER[key].name == Name) {
				USER_JSON_FTR_WALL_TWITTER[key].data = value;
				flag = false;
				break;
			}
		}
		if (flag) {
			var data = { "name": Name, "data": value };
			USER_JSON_FTR_WALL_TWITTER.push(data);
		}
		localStorage.setItem(setName, JSON.stringify(USER_JSON_FTR_WALL_TWITTER));
	}
}
