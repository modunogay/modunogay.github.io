	...
	$(function () {
		$.get("./version/now.txt", function (data) {
			$.getJSON(`./json/${data}.json`, function (list) {
				...
				str = str.join("_")
				var url = "http://" + location.host + "/MISC/index.html?" + str;
				$("#URL-Input").val(url);
			});
		})
	})
...
function setlocalstorage(Name, value, setName) {
	...
	if (setName == "MISC") {
	...
	}else if(setName == "MISC_TWITTER"){
		for (key in USER_JSON_MISC_TWITTER) {
			if (USER_JSON_MISC_TWITTER[key].name == Name) {
				USER_JSON_MISC_TWITTER[key].data = value;
				flag = false;
				break;
			}
		}
		if (flag) {
			var data = { "name": Name, "data": value };
			USER_JSON_MISC_TWITTER.push(data);
		}
		localStorage.setItem(setName, JSON.stringify(USER_JSON_MISC_TWITTER));
	}
}
